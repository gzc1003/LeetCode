...
num_to_column = [defaultdict(lambda:0) for i in xrange(N+1)]
...
for column in xrange(1, M+1):
    for up in xrange(N, 0, -1):
        for down in xrange(up, N+1):
            ...
            left_bound_tmp1 = max(num_to_column[down][matrix[up][column]], left_bound[up][down-1])
            left_bound_tmp2 = max(num_to_column[up][matrix[down][column]], left_bound[up+1][down])
